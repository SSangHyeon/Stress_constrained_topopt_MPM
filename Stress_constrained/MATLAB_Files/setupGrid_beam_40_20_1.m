function [lstps,g,mpData,mesh,mp] = setupGrid_beam_40_20_1
%% Analysis parameters
...
%   if (coord(node,1) < 1e-10) || (coord(node,2) < 20+1e-10)
%     bc(node*2-1,:)=[node*2-1 0];
%     bc(node*2  ,:)=[node*2   0];
%   end
%   if (coord(node,1) > 40+1e-10) || (coord(node,2) > 40+1e-10) || (coord(node,2) < 20+1e-10)
%     bc(node*2-1,:)=[node*2-1 0];
%     bc(node*2  ,:)=[node*2   0];
%   end
%   if coord(node,1)==40 && coord(node,2)== (29 || 30)                            % mid-depth pin
%     bc(node*2  ,:)=[node*2   0];
%     bc(node*2-1,:)=[node*2-1 0];
%   end
end
