function [lstps,g,mpData,mesh,mp] = setupGrid_L_bracket
%% Analysis parameters
E=12e6;   v=0.3;   fc=20e10;                                                 % Young's modulus, Poisson's ratio, yield strength   
...
nelsx=2*a;                                                                 % number of elements in the x direction
nelsy=2*a;                                                                 % number of elements in the y direction
ly=10;  lx=10;                                                              % domain dimensions
d=5;  l=5;                                                                 % beam dimensions
...
[etplmp,coordmp] = formCoord2D(4,4,l,d);                               % mesh for MP generation
coordmp(:,2)=coordmp(:,2)+(ly-d);                                           % adjust MP locations (vertical)
coordmp(:,1)=coordmp(:,1)+2;

%% make L-bracket
L = intersect(find(coordmp(:,2)>7),find(coordmp(:,1)>5));
coordmp(L,:) = [];
...
for mp = nmp:-1:1                                                           % loop backwards over MPs so array doesn't change size
  ...
  if mpC(mp,1) == max(mpC(:,1)) && mpC(mp,2) == 5 + abs(max(5-mpC(:,2)))
      mpData(mp).fp = [0 P].';
