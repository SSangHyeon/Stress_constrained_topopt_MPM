function [mesh,mpData,refernode,referelem] = elemMPinfo(mesh,mpData)
...
[nels,~] = size(mesh.etpl);                                                 % number of elements in mesh
...
%     end
    ...
    for i = 1:nn
        ...
        refernode((mp-1)*nn + i) = nIN(i); 
    end
    for i = 1:nne
        referelem((mp-1)*nne + i) = eIN(i);
    end
...
refernode = unique(refernode);
k = find(refernode);
refernode = refernode(k);
referelem = unique(referelem);
l = find(referelem);
referelem = referelem(l);
