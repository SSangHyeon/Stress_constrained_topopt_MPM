function [lstps,g,mpData,mesh,mp] = setupGrid_beam
...
for node=1:nodes                                                            % loop over nodes
  ...
  if coord(node,1)==0.5 && coord(node,2)==(ly-d/2)                            % mid-depth pin
    bc(node*2  ,:)=[node*2   0];
  end
end
...
[etplmp,coordmp] = formCoord2D(12*a,4*a,l,d);                           % mesh for MP generation
...
for mp = nmp:-1:1                                                           % loop backwards over MPs so array doesn't change size
  ...
  if abs(mpC(mp,1)-(l-lp(mp,1))-0.5)<1e-3/a && abs(mpC(mp,2)-(ly-d/5))<(lp(mp,2)+1e-3/a)
