function [lstps,g,mpData,mesh,mp0] = setupGrid_beam
...
%% Analysis parameters
E=12e6;   v=0.4;   fc=20e4;                                                 % Young's modulus, Poisson's ratio, yield strength   
...
mpSte = 0; % strain Energy of material point
%% Mesh generation
...
for node=1:nodes                                                            % loop over nodes
  if coord(node,1)==  0.5                                                       % roller (x=0)
    bc(node*2-1,:)=[node*2-1 0];
    bc(node*2,:) = [node*2 0];
  end
%   if coord(node,1)==l/2 + 0.5 && coord(node,2)==(ly-d)                            % mid-depth pin
%     bc(node*2  ,:)=[node*2   0];
%   end
end
...
for mp = nmp:-1:1                                                           % loop backwards over MPs so array doesn't change size
  ...
  if abs(mpC(mp,1)- lp(mp,1) - 0.5)<1e-3/a && abs(mpC(mp,2)-(ly/2-d))<(lp(mp,2)+1e-3/a) 
    mpData(mp).fp   = [0 0.1*P].';                                            % point forces at material points (end load)  
  elseif abs(mpC(mp,1) + lp(mp,1) - (l+0.5))<1e-3/a && abs(mpC(mp,2)-(ly/2-d))<(lp(mp,2)+1e-3/a)
    mpData(mp).fp   = [0 0.1*P].';
  else
    mpData(mp).fp   = zeros(nD,1);                                          % point forces at material points
  end
  mpData(mp).fp   = zeros(nD,1);  
  ...
  mpData(mp).mpSte = mpSte;     % Strain Energy by myself %%%%%%
end
end