function [lstps,g,mpData,mesh,nmp] = setupGrid_collapse
...
%% Analysis parameters
E=1e6;   v=0.4;   fc=20e3;                                                  % Young's modulus, Poisson's ratio, yield strength   
mCst=[E v fc];                                                              % material constants
P = -10e3;
g=10;                                                                       % gravity
rho=1000;                                                                   % material density
lstps=30;                                                                   % number of loadsteps
...
mp=9;                                                                       % number of material points in each direction per element
...
%% Mesh generation
[etpl,coord] = formCoord2D(100*nelsx,nelsy,100*lx,ly);                          % background mesh generation
...
%   if coord(node,1)==0 %|| coord(node,1) == 9                                                      % roller (x=0)
%     bc(node*2-1,:)=[node*2-1 0];    
%   end
  if coord(node,2)==0 %|| coord(node,1) == 0                                                      % roller (y=0)
    bc(node*2  ,:)=[node*2   0];
  end
end
...
for mp = nmp:-1:1                                                           % loop backwards over MPs so array doesn't change size
  ...
  mpData(mp).mCst   = mCst;                                                 % material constants (or internal variables) for constitutive model
   if abs(mpC(mp,1) - lx) < lp(mp,1)+((1e-3)/a) && abs(mpC(mp,2)-(ly/2))<(lp(mp,2)+((1e-3)/a)) %-d/2
    mpData(mp).fp   = [P 0].';                                            % point forces at material points (end load)  
  else
