function [lstps,g,mpData,mesh,mp] = setupGrid_beam_revised
...
%% Analysis parameters
%E = 915e6; v = 0.443; fc = 500e6;
E=12e6;   v=0.3;   fc=20e4;                                                 % Young's modulus, Poisson's ratio, yield strength   
mCst=[E v fc];                                                              % material constants
g=10;                                                                       % gravity
rho=1;                                                                      % material density
P=-10e3;                                                                   % applied end load
lstps = 50;                                                                   % number of loadsteps
a = 2;                                                                      % element multiplier
nelsx=22*a; %22                                                                 % number of elements in the x direction
nelsy=20*a; %20                                                                % number of elements in the y direction
...
for node=1:nodes                                                            % loop over nodes
  if coord(node,1)==0.5 + 5 && (coord(node,2) >= 8 && coord(node,2) <=9)                                                     % roller (x=0)
    bc(node*2-1,:) = [node*2-1 0];    
  end
  if coord(node,1)==0.5 + 5 && coord(node,2)== 8                            % mid-depth pin
    bc(node*2  ,:) = [node*2   0];
  end
%   if coord(node,1)==0 || coord(node,1) == lx
%       bc(node*2-1,:) = [node*2-1 0];
%   end
%   if coord(node,2)==0 || coord(node,2) == ly
%       bc(node*2,:) = [node*2 0];
%   end
end
...
coordmp(:,2)=coordmp(:,2)+(ly-d-1);                                         % adjust MP locations (vertical)
coordmp(:,1)=coordmp(:,1)+0.5;
...
for mp = nmp:-1:1                                                           % loop backwards over MPs so array doesn't change size
  ...
  if abs(mpC(mp,1)-(l-lp(mp,1))-0.5)<1e-3/a && abs(mpC(mp,2)-(ly-1-d))<(lp(mp,2)+1e-3/a)
    mpData(mp).fp   = [-3*P*0.5 5*P*0.5].';                                            % point forces at material points (end load)  
  elseif abs(mpC(mp,1)-lp(mp,1)-0.5)<1e-3/a && abs(mpC(mp,2)-(ly-1-d))<(lp(mp,2)+1e-3/a)
      mpData(mp).fp = [3*P*0.5 5*P*0.5].';
  else
  ...
  mpData(mp).ang = 0;
  mpData(mp).jacob = det(mpData(mp).F);
  mpData(mp).R = eye(3);                                                  % Rotation tensor
  mpData(mp).U = zeros(3);
end
