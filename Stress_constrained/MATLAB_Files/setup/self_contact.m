function [lstps,g,mpData,mesh,mp0] = self_contact
...
%   if coord(node,1) == l/2                                                   % roller (x=l/2)
%     bc(node*2,:)=[node*2 0];    
%   end
  if coord(node,1) == (l/2) && coord(node,2)== 0                            % mid-depth pin
    bc(node*2, :) = [node*2 0];
...
for mp = nmp:-1:1                                                           % loop backwards over MPs so array doesn't change size
  ...
  if (abs(mpC(mp,1)-(l-lp(mp,1)))<1e-6) && (abs(mpC(mp,2)-(0))<(lp(mp,2)+1e-3/a)) ...
          || (abs(mpC(mp,1)-lp(mp,1))<1e-6) && (abs(mpC(mp,2)-(0))<(lp(mp,2)+1e-3/a))
    mpData(mp).fp   = [0 -P/10].';                                            % point forces at material points (end load)   
  else
